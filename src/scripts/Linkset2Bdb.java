...
public class Linkset2Bdb {
	...
	public static void main(String[] args) throws IDMapperException, SQLException, InterruptedException, IOException {
		// TODO Auto-generated method stub
		//Initialize BrideDb data source
		DataSourceTxt.init();		
		//Path of the directory which contains the linksets
		String url = "http://data.openphacts.org/1.5/ims/linksets/HMDB/";
		File dir = new File(url.replace("http://", ""));
		if (!dir.exists()){
			System.out.println("wget start");
			Process p = Runtime.getRuntime().exec("wget -r --no-parent "+url);
			p.waitFor();
			System.out.println("wget done");
		}
		if (dir.isDirectory()){
	...
	public static void parse(Model model){
		...
		while (list.hasNext()){
			...
			if (s.getPredicate().getURI().equals("http://www.w3.org/2004/02/skos/core#relatedMatch")){			
				...
				DataSource ds_sub = DataSource.getByIdentiferOrgBase(db_sub);
				DataSource ds_obj = DataSource.getByIdentiferOrgBase(db_obj);
				if (ds_sub != null && ds_obj != null ){
					// TODO How to handle URIs which are not from IdentiferOrg ?
					Xref mainXref = new Xref(ref_sub,ds_sub) ;
					Xref xref = new Xref(ref_obj,ds_obj);
